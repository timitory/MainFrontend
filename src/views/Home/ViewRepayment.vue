<template>
  <div>
    <div class="mt-8 px-6 pt-6 lg:px-20 pb-20 relative shadow-lg bg-white whitebox" style="box-shadow: 0px 20px 33px #00000029;">
      <router-link to="/app/dashboard/managehome" class="absolute left-4 top-2">
      ...
      <div class="overflow-x-auto lg:overflow-x-hidden tablecont">
        <table class="w-full mt-8">
          <thead>
            <tr>
              <th class="font-semibold">Payment period</th>
      ...
      <div class="mt-24 ">
        <router-link class="text-green-500 block" to="/app/dashboard/managehome">Back</router-link>
      </div>
    </div>
  ...
  methods: {
    payRepayment(id){
      this.$store.commit('startLoading')
        axios({url: `${baseURL}/homecontent/pay`, data:{repayment_id : id}, method: 'POST'})
        .then(res=>{
          // console.log(res)
