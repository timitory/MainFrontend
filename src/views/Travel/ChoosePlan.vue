      ...
      <p class="font-semibold">Choose your preferred Insurance provider?</p>
        <div class="mt-8 box">
          <label v-for="(item, index) in underwriters" :key="index" class=" border border-gray-300 text-lg border-solid" :class="[item.id !== 2 && item.id !== 6 ? 'opacity-50 hidden' : 'container']">
            ...
            <span class="tooltiptext text-sm shadow" v-if="item.id !== 2 && item.id !== 6">{{item.name}} is coming soon</span>
          </label>
        </div>
    </div>
  </div>
  ...
  watch:{
    underwriter(){
      if(this.underwriter !== ''){
        ...
        if(this.underwriter === 2) {
        this.$router.push('/app/dashboard/buytravel/2')

        }else if(this.underwriter === 6) {
        this.$router.push('/app/dashboard/buytravel/allianz')
        }
        // this.$store.commit('setUnderwriterId', this.underwriter)
      }
    }
