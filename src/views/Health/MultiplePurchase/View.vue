<template>
  <div class="mt-8">
    <div class="">
      ...
      <button @click="submit" class="mt-10 bg-green-500 text-white block lg:inline focus:outline-none px-4 py-2 lg:px-10 rounded float-right" :class="[ !formValid ? 'opacity-30' : '']" :disabled="!formValid">Proceed to payment</button>
    </div>
    <VirtualAccount v-if="showVirtual" />
...
import VirtualAccount from "@/views/Health/MultiplePurchase/VirtualAccount.vue";
export default {
  components: {VirtualAccount, Paystack, PaymentMethod, UpdateEnrolleeModal},
  props: {
    reference : {
      required: true
    },
  ...
  methods: {
    submit() {
      if(localStorage.getItem('PaymentMode') === 'Card') {
