      ...
      <form @submit.prevent="submit" class="py-6 lg:pb-20" style="min-height: 30vh">
        ...
        <div class="lg:w-full lg:mr-3 mb-6">
          <div class="">
                ...
                <input v-model="name" type="text" class="rounded block mt-4 bg-blue-100 px-4 py-2 w-full outline-none focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent" placeholder="" style=" background-color: #e2f5ec;" required>
            </div>
            <div class="">
                <label class="text-sm font-bold">Company Email</label>
                <input v-model="email" type="email" class="rounded block mt-4 bg-blue-100 px-4 py-2 w-full outline-none focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent" placeholder="" style=" background-color: #e2f5ec;" required>
            </div>
            <div class="">
                <label class="text-sm font-bold">Company Phone</label>
                <input v-model="telephone" type="text" class="rounded block mt-4 bg-blue-100 px-4 py-2 w-full outline-none focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent" placeholder="" style=" background-color: #e2f5ec;" required>
          ...
          <label class="text-sm font-bold">Select Payment Method</label>
          <div class="relative">
            <select v-model="paymentMode" class="rounded block mt-4 bg-blue-100 px-4 py-2 w-full outline-none focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent" required style=" background-color: #e2f5ec;">
              <option value="Card">Pay with Card</option>
              <option value="virtual">Pay with virtual Account</option>
            </select>
          </div>
        </div>
        <input type="file" ref="excelfile" id='file' class="hidden" v-on:change="upload">
        <label class="text-sm font-bold">Upload Excel Document</label>
...
export default {
    ...
    methods:{
      ...
      submit(){
          ...
          formData.append('corporate_name', this.name);
          formData.append('corporate_phone', this.telephone);
          formData.append('corporate_email', this.email);
          ...
          .then(res =>{
              ...
              this.$router.push({
                ...
                query: {
                  ...
                  user_health_id : res.data.data.user_health_id
                }
              })
            let data = {
              account_name: res.data.data.virtual_account_details.account_name,
              account_number: res.data.data.virtual_account_details.account_number,
              bank_name: res.data.data.virtual_account_details.bank_name,
              amount: res.data.data.totalamount
            }
            this.$store.commit('setVirtualAccount', data )
          })
          .catch(err => {
