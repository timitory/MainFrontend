<template>
    <div class="mt-8">
      <div class="lg:w-3/4 lg:mx-auto">
        <p class="font-semibold">Choose your preferred Insurance provider?</p>
          <div class="mt-8 box">
            <label v-for="(item, index) in underwriters" :key="index" class="container-hh border border-gray-300 text-lg border-solid" :class="[item.id !== 5 ? 'opacity-50' : '']">
              ...
              <span class="checkmark-hh"></span>
              <span class="tooltiptext text-sm shadow" v-if="item.id !== 5">{{item.name}} is coming soon</span>
            </label>
          </div>
      </div>
    </div>
    ...
    watch:{
      underwriter(){
        if(this.underwriter !== ''){
          ...
          this.$store.commit('setCoverForm', 3)
  ...
  .container-hh input:checked ~ .checkmark-hh {
    background-color:#00A859;
    opacity: 1
  }
  ...
  .container-hh input:checked ~ .checkmark:after {
    display: block;
  }
  
  /* Style the checkmark/indicator */
  .container-hh .checkmark-hh:after {
    left: 9px;
    top: 5px;
    width: 7px;
    height: 12px;
  ...
  .container-hh:hover .tooltiptext {
    visibility: visible;
  }
  .name{
    font-size: 0.8em;
